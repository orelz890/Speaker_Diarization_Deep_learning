...
# The recipe consists in doing Agglomerative Hierachical Clustering
# (explained here: https://www.datanovia.com/en/lessons/agglomerative-hierarchical-clustering/)
# on x-vectors in a first step. Then, Variational Bayes HMM over x-vectors
# (paper- https://arxiv.org/pdf/2012.14952.pdf)
...
#                 time_end = time.time()
#                 print("\n=========", "It took ", (time_end - time_start), " to finish the task", "\n=========")
    ...
    for file_name, segs in recit:
        ...
        if args.init == 'AHC' or args.init.endswith('VB'):
            if args.init.startswith('AHC'):
                ...
                #
                # kn = KneeLocator(range(0, len(scr_mx)), scr_mx, curve='convex', direction='decreasing')
                # lin_mat = fastcluster.linkage(
                #     scr_mx, method='average', preserve_input='False', )
                del scr_mx
                adjust = abs(lin_mat[:, 2].min())
